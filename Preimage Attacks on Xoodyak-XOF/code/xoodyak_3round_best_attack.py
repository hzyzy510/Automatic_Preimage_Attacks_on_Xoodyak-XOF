#!/usr/bin/env python3
"""Single MILP that mixes exact and linear 3-round Xoodyak attacks.

The model shares the propagation up to Y3, then lets every output position
choose one of two branches:

* linear: select linear bits in Y3[y=2] and use the last-chi approximation.
* exact:  propagate the third chi exactly and select linear bits in Z3[y=2].
...
STATES = ["X1", "Y1", "Z1", "X2", "Y2", "Z2", "X3", "Y3", "Z3"]
COND_STATES = ["Y1", "Y2", "Y3"]
GIVEN_GUESSED_BY_LANE = {
    0: [0, 1, 2, 4, 5, 6, 7, 8, 9, 10, 11, 13, 14, 15, 16, 18, 19, 20, 22, 23, 24, 25, 27, 28, 29, 30, 31],
    1: [1, 2, 3, 5, 6, 7, 8, 10, 11, 12, 13, 14, 15, 16, 17, 19, 20, 21, 22, 24, 25, 26, 28, 29, 30, 31],
    2: [0, 2, 3, 4, 6, 7, 8, 9, 11, 12, 13, 14, 15, 16, 17, 18, 20, 21, 22, 23, 24, 25, 26, 27, 29, 30, 31],
    3: [0, 1, 3, 4, 5, 6, 7, 8, 9, 10, 12, 13, 14, 15, 17, 18, 19, 21, 22, 23, 24, 26, 27, 28, 29, 30, 31],
}
...
GIVEN_SELECTED_BY_LANE = {
    ...
    1: [6,15,24,25,29],
    2: [8,17,21,30],
    3: [4,9,13,18,22,27,31],
}

GIVEN_CONDITIONS_EQ_0 = {
    "Y1": {
        0: {0: [7,30], 1:[8]},
        1: {0: [4,9,13,18,22,27,31], 1: [0,5,14,19,23], 2:[2,6,11,15,29], 3:[3,7,12,16,21,30]},
    },
}

GIVEN_CONDITIONS_EQ_1 = {
    "Y1": {
        0: {0:[0,5,28], 1: [11,20,29], 2: [7,12,16,21,30], 3:[4,27,31]},
        1: {0:[2,6,7,11,16,20,25,30], 1: [3,7,8,12,17,21,26,30], 2: [0,4,9,18,23,27], 3: [1,5,10,14,19,24,28]},
        2: {0:[4,9,13,18,22,27,31], 1:[0,5,14,19,23,28], 2: [2,6,11,15,29], 3: [3,7,12,16,30]},
    },
}
...
def build_model(
    ...
    model = gp.Model("xoodyak_3round_best_exact_or_linear")
    ...
    selected_linear = model.addVars(LANES, LANE_BITS, vtype=GRB.BINARY, name="SEL_LINEAR_Y3")
    selected_exact = model.addVars(LANES, LANE_BITS, vtype=GRB.BINARY, name="SEL_EXACT_Z3")
    ...
    weighted_conditions = y1_conditions + 2.5 * y2_conditions + 32 * y3_conditions
    condition_count = y1_conditions + y2_conditions + y3_conditions

    for x in range(LANES):
        for z in range(LANE_BITS):
            model.addConstr(selected_linear[x, z] <= lin["Y3", 2, x, z], name=f"sel_linear_y3_{x}_{z}")
            model.addConstr(selected_exact[x, z] <= lin["Z3", 2, x, z], name=f"sel_exact_z3_{x}_{z}")
...
def collect_solution(data: Dict[str, object]) -> Dict[str, object]:
    ...
    final_linear_y3 = [(x, z) for x in range(LANES) for z in range(LANE_BITS) if bit(lin["Y3", 2, x, z])]
    final_linear_z3 = [(x, z) for x in range(LANES) for z in range(LANE_BITS) if bit(lin["Z3", 2, x, z])]
    ...
    return {
        ...
        "final_linearizable_y3_count": len(final_linear_y3),
        "final_linearizable_z3_count": len(final_linear_z3),
        ...
        "final_linearizable_y3_by_lane": by_lane(final_linear_y3),
        "final_linearizable_z3_by_lane": by_lane(final_linear_z3),
...
def write_mixed_svg(path: Path, data: Dict[str, object], result: Dict[str, object]) -> None:
    ...
    def color(state: str, p: int, x: int, z: int) -> str:
        if state in COND_STATES:
            ...
            if bit(cond1[state, p, x, z]):
                return "#2da44e"
        if state == "Y3" and p == 2 and bit(selected_linear[x, z]):
            return "#8ec5ff"
        if state == "Z3" and p == 2 and bit(selected_exact[x, z]):
            return "#b197fc"
    ...
    lines = [
        ...
        f'<text x="20" y="22" class="title">Xoodyak mixed 3-round attack ({result["attack_type"]})</text>',
        '<rect x="20" y="34" width="11" height="11" fill="#ffd84d" stroke="#666"/><text x="36" y="43">linear variable</text>',
        '<rect x="134" y="34" width="11" height="11" fill="#d73a49" stroke="#666"/><text x="148" y="43">condition = 0</text>',
        '<rect x="231" y="34" width="11" height="11" fill="#2da44e" stroke="#666"/><text x="244" y="43">condition = 1</text>',
        '<rect x="20" y="50" width="11" height="11" fill="#b8b8b8" stroke="#666"/><text x="36" y="59">constant</text>',
        '<rect x="134" y="50" width="11" height="11" fill="#8ec5ff" stroke="#666"/><text x="148" y="59">linear sel</text>',
        '<rect x="231" y="50" width="11" height="11" fill="#b197fc" stroke="#666"/><text x="244" y="59">exact sel</text>',
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Single Xoodyak 3-round MILP mixing exact and linear output bits.")
    ...
    parser.add_argument("--svg-output", type=Path, default=Path("work/xoodyak_3round_best_attack_1_2.5_32_(1).svg"))
    ...
    for key in [
        ...
        "final_linearizable_y3_count",
        "final_linearizable_z3_count",
    ]:
